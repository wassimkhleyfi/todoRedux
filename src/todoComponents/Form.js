...
import * as actionTypes from '../JS/actions/actions';
...
const Form = ({ title, setTitle, addItem, editItem, edit, error, setError }) => {
    ...
    const handleChange = (event) => {
        ...
        setTitle(title);
        if(title.length === 0){
            setError("SVP écrire  la tache à faire");
        }else{
            setError("");
        }
    }
        ...
        <Container maxWidth="sm" className={classes.root}>
            <Grid container alignItems="center">
                <Grid item md={12}>
                    ...
                    error={!!error} helperText={error} id="outlined-basic" fullWidth label="Ecrire ici une tache à faire" multiline variant="outlined" />
                </Grid>
                <Grid item md={12}>
